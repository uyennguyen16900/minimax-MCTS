...
def evaluate(state, depth, winner):
...
def minimax(state, depth, player):
    ...
    if current_state == "Draw" or current_state == "Done":
        return {"score": evaluate(current_state, depth, winner), "move": "-1 -1"}
    ...
    for cell in empty_cells(state):
        ...
        state[x][y] = player
        if player == "O":
            result = minimax(state, depth, "X")
        else:
            result = minimax(state, depth, "O")
        state[x][y] = " "
...
if __name__ == "__main__":
    ...
    while playing:
        ...
        while state == "Not done":
            if player == "O":
                player_input = input("It's your turn. Please choose a position by entering 2 numbers seperated by a space to place your mark (e.g. 2 1): ")
                play_move(game_state, player, player_input)

            else:
                print("It's computer's turn.")
                position = minimax(game_state, 0, player)
